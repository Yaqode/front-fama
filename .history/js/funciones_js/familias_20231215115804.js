...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Agrega un evento al botón para que se despliegue al pasar el ratón
    var dropdownButton = document.getElementById("familiasDropdown");
...
function obtenerListaFamilias() {
    ...
    fetch('http://localhost:8090/fama-market/api/families/all')
        ...
        .then(familias => {
            ...
            actualizarMenuDesplegable('familiasMenu', familias, 'obtenerCategoriasYSubcategorias');
        })
        .catch(error => {
...
function obtenerCategoriasYSubcategorias(familiaId) {
    ...
    fetch(`http://localhost:8090/fama-market/api/categories-and-subcategories/byFamily/${familiaId}`)
        ...
        .then(categoriasYSubcategorias => {
            // Muestra categorías y subcategorías en el menú desplegable de la familia
            actualizarMenuDesplegable(`familiaMenu${familiaId}`, categoriasYSubcategorias, null);
        })
        .catch(error => {
...
function actualizarMenuDesplegable(menuId, items, nextFunction) {
