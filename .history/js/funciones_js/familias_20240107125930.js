...
function obtenerLista() {
    // Obtener todas las claves almacenadas en sessionStoreNg
    const todasLasClaves = sessionStoreNg.getAllKeys();

    // Verificar si se encontraron claves
    if (todasLasClaves.length === 0) {
        console.error('No se encontraron claves en sessionStoreNg');
        return;
    }

    // Tomar la primera clave (podrías adaptar esto según tus necesidades)
    const tokenKey = todasLasClaves[0];
    
    // Obtener el token desde sessionStoreNg
    const token = sessionStoreNg.get(tokenKey);
...
// Llamada a la función para obtener la lista al cargar la página
obtenerLista();
