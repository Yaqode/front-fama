...
function mostrarProductos(productos) {
    ...
    if (productos.length > 0) {
        ...
        if (primerProducto.images && primerProducto.images.length > 0) {
            primerProducto.images.forEach((imagen) => {
                ...
                // Agrega la imageId como texto alternativo
                const p = document.createElement('p');
                p.textContent = 'Image ID: ' + imagen.imageId;
                contenedorImagenes.appendChild(p);
            });
        }
