...
function guardarMarca() {
 ...
 // Función para obtener la imagen en formato base64
 function obtenerImagenBase64() {
    var inputImagen = document.getElementById('fileInput');
    if (inputImagen.files.length > 0) {
        var imagenFile = inputImagen.files[0];
        var reader = new FileReader();
        reader.readAsDataURL(imagenFile);
        reader.onload = function () {
            var base64String = reader.result.split(',')[1]; // Obtener solo la parte base64
            // Llamar a la función que maneja el guardado después de cargar la imagen
            // Esta es una opción si deseas que la llamada se realice después de cargar la imagen
            // En lugar de retornar el valor, podrías llamar a guardarMarca() aquí directamente.
            guardarMarca(base64String); // Llamada a guardarMarca con el base64String como parámetro
        };
    }
    return null;
}
