...
function obtenerListaFamilias() {
    ...
    fetch('http://localhost:8090/fama-market/api/families/all')
        .then(response => response.json())
        .then(familias => {
            // Muestra las familias en el menú desplegable
            actualizarMenuDesplegable('familiasMenu', familias);
        })
        .catch(error => {
...
function crearMenuDesplegable(menuId, items) {
    // Crea un nuevo menú desplegable
    var nuevoMenu = document.createElement('ul');
    nuevoMenu.className = 'dropdown-menu';

    // Agrega los elementos al nuevo menú
    items.forEach(function (item) {
        var menuItem = document.createElement('li');
        menuItem.textContent = item.name; // Ajusta según la estructura real de tu objeto
        nuevoMenu.appendChild(menuItem);
    });

    // Reemplaza el contenido del menú existente con el nuevo menú desplegable
    var menuExistente = document.getElementById(menuId);
    menuExistente.innerHTML = '';
    menuExistente.appendChild(nuevoMenu);
}

function obtenerCategoriasYSubcategorias(familiaId) {
    // Realiza la solicitud para obtener tanto categorías como subcategorías asociadas a la familia
    fetch(`http://localhost:8090/fama-market/api/categories-and-subcategories/byFamily/${familiaId}`)
        .then(response => response.json())
        .then(categoriasYSubcategorias => {
            // Crea un nuevo menú desplegable para categorías y subcategorías
            crearMenuDesplegable(`familiaMenu${familiaId}`, categoriasYSubcategorias);
        })
        .catch(error => {
