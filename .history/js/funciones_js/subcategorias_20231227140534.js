...
function obtenerListaSubcategorias() {
    fetch("http://localhost:8090/fama-market/api/subcategories/all")
        ...
        .then((subcategorias) => {
            // Obtener la referencia de la tabla y del área de paginación
            var tablaSubcategorias = document.getElementById('tablaSubcategorias').getElementsByTagName('tbody')[0];
            var paginacion = document.getElementById('paginacion');

            // Limpiar filas existentes (si las hay) y contenido de paginación
            tablaSubcategorias.innerHTML = '';
            paginacion.innerHTML = '';

            // Calcular el número total de páginas
            var totalPaginas = Math.ceil(subcategorias.length / elementosPorPagina);

            // Calcular el índice inicial y final de las subcategorías a mostrar
            var inicio = (paginaActual - 1) * elementosPorPagina;
            var fin = Math.min(inicio + elementosPorPagina, subcategorias.length);
            ...
            for (var i = 1; i <= totalPaginas; i++) {
                var numeroPagina = document.createElement('span');
                numeroPagina.textContent = i;
                numeroPagina.classList.add('numero-pagina');

                // Agregar un evento de clic para cambiar la página al hacer clic en un número de página
                numeroPagina.addEventListener('click', function (event) {
...
// Llamar a la función para obtener y mostrar la lista al cargar la página
obtenerListaSubcategorias();
