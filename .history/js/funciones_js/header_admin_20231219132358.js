document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("http://localhost:8090/fama-market/api/subcategories/all")
    ...
    .then((data) => {
      ...
      // Construir el menú// ... (código existente) ...
...
familiasMap.forEach((categoriasMap, familiaName) => {
    ...
    categoriasMap.forEach((subcategoriasArray, categoriaName) => {
        ...
        const categoriaContainer = document.createElement("div"); // Contenedor externo para la categoría
        const categoriaContent = document.createElement("div"); // Contenedor para el contenido
        categoriaContainer.appendChild(categoriaContent);
        categoriaItem.appendChild(categoriaContainer);

        // Ajustar el tamaño de las categorías (ajusta el valor según tus necesidades)
        categoriaContainer.style.height = "50px"; // Modifica el tamaño aquí

        categoriaContent.innerHTML = `<a href="#">${categoriaName}</a>`;

        if (subcategoriasArray.length > 0) {
            const subcategoriasList = document.createElement("ul");

            // Ajustar el tamaño de las subcategorías (ajusta el valor según tus necesidades)
            subcategoriasList.style.height = "30px"; // Modifica el tamaño aquí

            subcategoriasArray.forEach((subcategoriaName) => {
                const subcategoriaItem = document.createElement("li");
                const subcategoriaContainer = document.createElement("div"); // Contenedor externo para la subcategoría
                const subcategoriaContent = document.createElement("div"); // Contenedor para el contenido
                subcategoriaContainer.appendChild(subcategoriaContent);
                subcategoriaItem.appendChild(subcategoriaContainer);

                subcategoriaContainer.style.height = "30px"; // Modifica el tamaño aquí
                subcategoriaContent.innerHTML = `<a href="#">${subcategoriaName}</a>`;
                subcategoriasList.appendChild(subcategoriaItem);
            });

            categoriaContainer.appendChild(subcategoriasList);
        }
