...
function cargarFamilias() {
    fetch('http://localhost:8090/fama-market/api/subcategories/all')
        .then(response => response.json())
        .then(data => {
            const dropdown = document.getElementById('miDropdownMenu');

            data.forEach(familia => {
                const listItem = document.createElement('li');
                listItem.innerHTML = `<a class="trigger right-caret">${familia.family.nameFamily}</a>`;
                
                const categoriasDropdown = document.createElement('ul');
                categoriasDropdown.className = 'dropdown-menu sub-menu';
                ...
                listItem.appendChild(categoriasDropdown);
